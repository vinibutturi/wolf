...
--
Proxmox LXC + Nvidia (manual)::
+
--

[WARNING,caption=WARNING]
====
At the moment it is only possible to run Wolf in inside a privileged LXC.
====
First you need to make sure your Nvidia drivers (version >= 530.30.02) are properly installed and loaded on your host.
...
|  0%   39C    P8              9W /  165W |       2MiB /  16380MiB |      0%      Default |
|                                         |                        |                  N/A |
+-----------------------------------------+------------------------+----------------------+
...
`ls -l /dev/nividia*`
```
crw-rw-rw- 1 root root 195,   0 Aug 15 10:44 /dev/nvidia0
crw-rw-rw- 1 root root 195, 255 Aug 15 10:44 /dev/nvidiactl
crw-rw-rw- 1 root root 195, 254 Aug 15 10:44 /dev/nvidia-modeset
crw-rw-rw- 1 root root 506,   0 Aug 15 10:44 /dev/nvidia-uvm
crw-rw-rw- 1 root root 506,   1 Aug 15 10:44 /dev/nvidia-uvm-tools

/dev/nvidia-caps:
total 0
cr-------- 1 root root 509, 1 Aug 15 10:44 nvidia-cap1
cr--r--r-- 1 root root 509, 2 Aug 15 10:44 nvidia-cap2
```

.I am not able to see all the Nvidia devices
[%collapsible]
====
...
Add udev rules to add missing Nvidia devices in PVE 8
...
`reboot` you PVE host and try running `ls -l /dev/nividia*` again.
====


Add the udev rules to the host as explained in the xref:quickstart.adoc#_virtual_devices_support[Virtual devices support] section.

One last final check on the host: we have to make sure that the `nvidia-drm` module has been loaded and that the module is loaded with the flag `modeset=1`.
...
The easiest way to make the change persistent is to add `nvidia-drm.modeset=1` to the `GRUB_CMDLINE_LINUX_DEFAULT` line in `/etc/default/grub` ex:
